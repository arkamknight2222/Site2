...
export default function JobApplicants() {
  ...
  const [pendingStatusChange, setPendingStatusChange] = useState<{applicant: ExtendedApplicant, newStatus: ApplicationStatus} | null>(null);
  const [sortCriteriaStatus, setSortCriteriaStatus] = useState<SortCriterion[]>([]);
  const [sortCriteriaRandom, setSortCriteriaRandom] = useState<SortCriterion[]>([]);
  const [sortCriteriaPoints, setSortCriteriaPoints] = useState<SortCriterion[]>([]);
  ...
  const handleStatusChangeRequest = (applicant: ExtendedApplicant, newStatus: ApplicationStatus) => {
  ...
  const sortApplicants = (applicants: ExtendedApplicant[], criteria: SortCriterion[]): ExtendedApplicant[] => {
    if (criteria.length === 0) return applicants;
      ...
      for (const criterion of criteria) {
        ...
        switch (criterion.field) {
          ...
          case 'date':
            ...
            break;
          case 'experience':
            const expLevels = ['Entry', 'Mid', 'Senior', 'Expert'];
            const aExpLevel = expLevels.findIndex(level => a.experience?.includes(level));
            const bExpLevel = expLevels.findIndex(level => b.experience?.includes(level));
            comparison = aExpLevel - bExpLevel;
            break;
          case 'degree':
            const degreeLevels = ['High School', 'Associate', 'Bachelor', 'Master', 'PhD'];
            const aDegreeLevel = degreeLevels.findIndex(level => a.degree?.includes(level));
            const bDegreeLevel = degreeLevels.findIndex(level => b.degree?.includes(level));
            comparison = aDegreeLevel - bDegreeLevel;
            break;
          case 'status':
            const statusOrder: ApplicationStatus[] = ['applicant', 'interested', 'in_review', 'interviewing', 'interviewed', 'offer_extended', 'accepted', 'rejected', 'waitlisted'];
            comparison = statusOrder.indexOf(a.applicationStatus) - statusOrder.indexOf(b.applicationStatus);
            break;
        }
  ...
  const filterApplicantsNoFilter = (applicants: ExtendedApplicant[], searchTerm: string) => {
    return applicants.filter(applicant => applicant.name.toLowerCase().includes(searchTerm.toLowerCase()));
  };
  ...
  const filteredStatusApplicants = sortApplicants(
    filterApplicantsNoFilter(
  ...
  const filteredRandomApplicants = sortApplicants(
    filterApplicants(randomApplicants, searchRandom),
    sortCriteriaRandom
  );

  const filteredPointsApplicants = sortApplicants(
    filterApplicants(pointsApplicants, searchPoints),
    sortCriteriaPoints
  );
  ...
  const statusSortFields = [
    { field: 'name' as const, label: 'Name' },
    { field: 'date' as const, label: 'Date Applied' },
    { field: 'experience' as const, label: 'Experience' },
    { field: 'degree' as const, label: 'Education' },
    { field: 'status' as const, label: 'Status Type' }
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="text-right border-l border-gray-200 pl-8">
              <p className="text-sm text-gray-600">Reviewed Applicants</p>
              <p className="text-3xl font-bold text-emerald-600">{statusApplicants.length}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center mb-6">
        <div className="flex items-center space-x-3 bg-white rounded-full px-6 py-3 shadow-md border border-gray-200">
          <button
      ...
      {currentSlide === 0 && statusApplicants.length > 0 && (
        <div className="bg-white rounded-2xl shadow-lg border border-emerald-200 overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200 bg-gray-50">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <MultiSortControl
                criteria={sortCriteriaStatus}
                onChange={setSortCriteriaStatus}
                availableFields={statusSortFields}
                themeColor="emerald"
              />
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-200">
                <div className="flex gap-2">
                  ...
                  <MultiSortControl
                    criteria={sortCriteriaRandom}
                    onChange={setSortCriteriaRandom}
                    availableFields={poolSortFields}
                    themeColor="gray"
                  />
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-blue-200 overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-200">
                <div className="flex gap-2">
                  ...
                  <MultiSortControl
                    criteria={sortCriteriaPoints}
                    onChange={setSortCriteriaPoints}
                    availableFields={poolSortFields}
                    themeColor="blue"
                  />
                </div>
